...
deltaBronzeTable = DeltaTable.forPath(spark, bronze_table_path)

fullHistoryDF = deltaBronzeTable.history()    # get the full history of the table
display(fullHistoryDF)

lastOperationDF = deltaBronzeTable.history(1) # get the last operation
...
deltaSilverTable = DeltaTable.forPath(spark, silver_table_path)
...
deltaSilverTable.alias("silver").merge(
    source = back_fill_df.alias("updates"),
    condition = "silver.id = updates.id"
...
from delta.tables import *
deltaSilverTable.alias("silver").merge(source = back_fill_df.alias("updates"),condition = "silver.id = updates.id").whenMatchedUpdateAll().whenNotMatchedInsertAll().execute()

# COMMAND ----------

display(deltaSilverTable.history())
display(deltaBronzeTable.history())
