...
display(df.select(col("device_operational_status")).distinct())
...
display(dbutils.fs.ls(f"{bronze_table_path}/_delta_log"))

# COMMAND ----------

import pprint
import json
#dbutils.fs.head("dbfs:/FileStore/yas_mokri/bootcamp_data/tables/bronze/_delta_log/00000000000000000000.json")
delta_log_df = spark.read.json("dbfs:/FileStore/yas_mokri/bootcamp_data/tables/bronze/_delta_log/00000000000000000000.json")
display(delta_log_df)
...
test_df = commitInfo_df.select(col("add.*"))

# COMMAND ----------

from pyspark.sql.functions import from_json,col
rdd = sc.parallelize([str(test_df.select("stats"))])
json_column = spark.read.json(rdd, primitivesAsString ='true')
display(json_column)


# COMMAND ----------

commitInfo_df = delta_log_df.select("commitInfo").where("commitInfo is not NULL")
display(commitInfo_df.select(col("commitInfo.*")))

# COMMAND ----------

display(commitInfo_df
        .select(col("commitInfo.*"), col("commitInfo.operationMetrics.*"))
        .drop("operationMetrics"))

# COMMAND ----------

from pyspark.sql.functions import to_json
col_df = col_df.withColumn( "test" , to_json(col_df.commitInfo).alias("test"))
...
# MAGIC val delta_log_df = spark.read.json("dbfs:/FileStore/yas_mokri/bootcamp_data/tables/bronze/_delta_log/00000000000000000000.json")
# MAGIC val json_schema = spark.read.json(delta_log_df.select("commitInfo").where("commitInfo is not NULL").as[String]).schema
# MAGIC //df.withColumn("jsonData", from_json($"jsonData", json_schema))

# COMMAND ----------

from pyspark.sql.functions import from_json,col
rdd = sc.parallelize([test_df])
json_column = spark.read.json(rdd, primitivesAsString ='false')
display(json_column)
